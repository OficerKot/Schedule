...
class ReportsController {
  ...
  constructor() {
    ...
    this.weekSelect = document.getElementById(
      "weekSelect",
    ) as HTMLSelectElement | null;
    this.semesterSelect = document.getElementById(
      "semesterSelect",
    ) as HTMLSelectElement | null;
    this.subdivisionSelect = document.getElementById(
      "subdivisionSelect",
    ) as HTMLSelectElement | null;
    this.buildingSelect = document.getElementById(
      "buildingSelect",
    ) as HTMLSelectElement | null;
  ...
  private handleLoadClick(): void {
    ...
    const semesterId = this.semesterSelect
      ? parseInt(this.semesterSelect.value)
      : 0;
    const subdivisionId = this.subdivisionSelect
      ? parseInt(this.subdivisionSelect.value)
      : 0;
    ...
    if (
      this.reportType === "matrix" &&
      this.groupSelect &&
      this.groupSelect.multiple
    ) {
      groupIds = Array.from(this.groupSelect.selectedOptions).map((opt) =>
        parseInt(opt.value),
      );
  ...
  private renderMatrix(data: MatrixData): void {
    ...
    if (!data || !data.groups || data.groups.length === 0) {
      this.content.innerHTML =
        '<div class="empty">Нет данных для шахматки</div>';
      return;
    }

    // Берём только первые 6 дней (ПН-СБ) и первые 8 периодов
    ...
    days.forEach((day) => {
      periods.forEach((p) => {
        tableHtml += `<th>${day}<br><small>${p}</small></th>`;
      });
    });
    ...
    groups.forEach((group) => {
      tableHtml += `<tr><td class="fixed-col"><strong>${group}</strong></td>`;
      days.forEach((day) => {
        periods.forEach((p) => {
          const key = `${day}_${p}`;
          const value = matrix[group]?.[key] || "";
          const cls = value ? "occupied" : "free";
          tableHtml += `<td class="${cls}">${value || "·"}</td>`;
        });
      });
      tableHtml += `</tr>`;
    });
    ...
    const wrapperHtml = `<div class="table-wrapper">${tableHtml}</div>`;
    ...
    this.content.innerHTML = wrapperHtml + footerHtml;
  }
  ...
  private renderClassrooms(data: ClassroomUsage): void {
    ...
    if (!data || typeof data.totalRooms === "undefined") {
      this.content.innerHTML =
        '<div class="empty">Нет данных о помещениях</div>';
      return;
    }
